                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 md:gap-12 mb-12">
                    ...
                    <div>
                        ...
                        <div className="flex space-x-3 md:space-x-4">
                            <SocialIcon href="https://www.instagram.com/pytech_arena_hackathon?utm_source=qr&igsh=MWszbW1xNjlqcGNidg==" target="_blank" icon={<Instagram className="w-4 h-4 md:w-5 md:h-5" />} />
...
function SocialIcon({ icon, href = "#", target }: { icon: React.ReactNode, href?: string, target?: string }) {
    return (
        <a
            ...
            className="w-10 h-10 rounded-lg bg-white/5 border border-white/5 flex items-center justify-center text-gray-400 hover:bg-white/10 hover:text-white hover:border-white/20 transition-all transform hover:-translate-y-1"
        >
            {icon}
        </a>
    )
