    ...
    return (
        <section id="tracks" className="py-24 relative">
            <div className="container mx-auto px-6">
                ...
                >
                    <h2 className="text-4xl md:text-5xl font-black mb-6 tracking-tight text-white uppercase">
                    ...
                    <p className="text-gray-400 max-w-2xl mx-auto text-lg font-mono">
                        Progression: <span className="text-emerald-400">Beginners</span> → <span className="text-blue-400">Intermediate</span> → <span className="text-purple-400">Advanced</span>
                    </p>
                </motion.div>
                ...
                >
                    {tracks.map((track) => (
                        <motion.div
                            ...
                            variants={item}
                            whileHover={{ y: -10 }}
                            className={`cyber-card rounded-3xl group flex flex-col h-full bg-[#050510]/80 backdrop-blur-xl border border-white/10 ${track.border} ${track.isAdvanced ? 'ring-2 ring-purple-500/50 shadow-[0_0_30px_rgba(168,85,247,0.15)]' : ''}`}
                        >
                            {/* Animated Header */}
                            <div className="p-8 relative overflow-hidden">
                                ...
                                <div className="flex justify-between items-start mb-6 relative">
                                    <div>
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <span className="text-xs font-mono text-gray-400 uppercase tracking-widest">{track.level}</span>
                                        </div>
                                        <h3 className="text-3xl font-bold text-white mb-1 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-white group-hover:to-gray-400 transition-all">
                                            {track.title}
                                        </h3>
                                ...
                                <div className="space-y-4 font-mono text-sm">
                                    ...
                                    <div className="flex items-center text-gray-400">
                                        ...
                                        <span className="text-emerald-400 font-bold">{track.nextStep}</span>
                                    </div>
                                </div>
                            </div>

                            {/* Rewards Area - HIGHLIGHTED */}
                            <div className={`px-8 py-5 border-t border-white/5 ${track.isAdvanced ? 'bg-purple-500/10' : 'bg-white/[0.02]'}`}>
                                ...
                                <div className="space-y-2">
                                    {track.perks.map((perk, i) => (
                                        <div key={i} className={`flex items-center gap-2 text-xs ${track.isAdvanced ? 'bg-white/5 p-2 rounded-lg border border-white/5' : ''}`}>
                                            {!track.isAdvanced && <div className={`w-1 h-1 rounded-full bg-gradient-to-r ${track.color}`} />}
                                            <span className={track.isAdvanced ? "text-white font-bold" : "text-gray-400"}>
                                                {perk.split(':').map((part, index) => (
                                                    <span key={index} className={index === 1 ? "text-yellow-400 text-sm ml-1" : ""}>
                            ...
                            {/* Skills Area */}
                            <div className="px-8 py-6 border-t border-white/5 bg-black/20 flex-grow">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {track.skills.map((skill) => (
                                        <span key={skill} className="text-[10px] font-mono bg-[#050510] border border-white/10 px-3 py-1.5 rounded-lg text-gray-400 group-hover:text-white group-hover:border-white/20 transition-all">
                                            {skill}
                                        </span>
                                    ))}
                                </div>
                            ...
                            {/* Action Area */}
                            <div className="p-8 mt-auto border-t border-white/5 relative bg-white/[0.04]">
                                <div className="flex justify-between items-end">
                                    <div>
                                        <p className="text-gray-400 text-[10px] uppercase tracking-widest mb-1">Registration Fee</p>
                                        <div className="flex items-baseline gap-1">
                                            <p className="text-3xl font-black text-white leading-none">{track.price}</p>
                                            <span className="text-[10px] text-white/60 uppercase font-bold tracking-wider">/ Member</span>
                                        </div>
                                    </div>
                                    {track.isLocked ? (
                                        <div className="px-6 py-3 rounded-xl font-bold text-xs text-gray-400 bg-white/5 border border-white/10 flex items-center justify-center gap-2 cursor-not-allowed">
                                            <Lock size={14} />
                                            Locked
                                        </div>
                                    ) : (
                                        <Link
                                            ...
                                            className={`px-6 py-3 rounded-xl font-bold text-xs text-white bg-gradient-to-r ${track.color} shadow-lg hover:shadow-xl transition-all hover:scale-105 active:scale-95 uppercase tracking-wider block text-center`}
                                        >
                                            Register Now
                                        </Link>
                                    )}
